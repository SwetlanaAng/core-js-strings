...
function repeatString(value, count) {
  if (count > 0) return value.repeat(count);
  return '';
}
...
function removeFirstOccurrences(str, value) {
  const ans = str.indexOf(value);
  if (ans !== -1) return str.slice(0, ans) + str.slice(ans + value.length);
  return str;
}
...
function removeLastOccurrences(str, value) {
  const ans = str.lastIndexOf(value);
  if (ans !== -1) return str.slice(0, ans) + str.slice(ans + value.length);
  return str;
}
...
function findLongestWord(sentence) {
  ...
  const arrLength = arrWords.map((item) => item.length);
  return arrWords[arrLength.indexOf(Math.max(...arrLength))];
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arrWords.length; i += 1) {
    let revertWord = '';
    for (let j = 0; j < arrWords[i].length; j += 1) {
      revertWord = arrWords[i][j] + revertWord;
    }
    ans.push(revertWord);
  }
  return ans.join(' ');
